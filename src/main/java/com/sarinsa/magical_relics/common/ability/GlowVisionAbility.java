...
public class GlowVisionAbility extends BaseArtifactAbility<GlowVisionAbility.GlowVisionAbilityConfig> {
    ...
    public InteractionResult onUse( Level level, @Nullable LivingEntity abilityUser, ItemStack artifact, InteractionHand hand, @Nullable HitResult hitResult ) {
        if( abilityUser != null && !ArtifactUtils.isAbilityOnCooldown( artifact, this ) ) {
            ...
            List<LivingEntity> nearbyEntities = level.getEntitiesOfClass( LivingEntity.class, abilityUser.getBoundingBox().inflate( radius, radius, radius ) );
            
            if( !nearbyEntities.isEmpty() ) {
                // Skip the player.
                nearbyEntities.remove( abilityUser );
                
                for( LivingEntity livingEntity : nearbyEntities ) {
                    // Skip entities that are not within a spherical area of the radius.
                    if( livingEntity.distanceTo( abilityUser ) > radius )
                        continue;
                    
                    livingEntity.addEffect( new MobEffectInstance( MobEffects.GLOWING, getConfig().GLOW_VISION.effectDuration.get() ) );
                }
                level.playSound( null, abilityUser.blockPosition(), SoundEvents.ZOMBIE_VILLAGER_CONVERTED, SoundSource.PLAYERS, 1.0F, 0.9F + (level.random.nextFloat() / 3) );
